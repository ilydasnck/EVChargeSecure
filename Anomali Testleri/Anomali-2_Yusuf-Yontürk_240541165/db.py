...
def add_event(event_type: str, target: str = "*", event_data=None):
    ...
    # Basic schema validation: reject negative durations
    try:
        # Accept both 'duration' and 'Duration' keys
        if isinstance(event_data, dict):
            for key in ['duration', 'Duration']:
                if key in event_data:
                    val = event_data[key]
                    if isinstance(val, (int, float)) and val < 0:
                        raise ValueError("Invalid record: duration cannot be negative")
...
def get_event(
    event_type: str, target: str = "*", first_acceptable_id: int = 1
...
def get_events(target: str = "*", data: dict = {}) -> str:
    ...
    try:
        # Get first un-executed event by event_type and target
        raw_data = cursor.execute(
            "SELECT * FROM Events WHERE target='"
            + target
            + "' and data='"
            + text_json
            + "';"
...
def get_cps(target: str = "*", data: dict = {}) -> str:
    cursor = _db.cursor()
    text_json = json.dumps(data).replace("%20", " ")
    target = target.replace("%20", " ")
    try:
...
def auth_user(user: str, password: str) -> bool:
    # WARNING: Demo purpose only. Real systems must use salted password hashes.
    cursor = _db.cursor()
    try:
        # Gets the password of the user if exists
        raw_data = cursor.execute(
...
def check_user(user: str) -> str:
    # Returns user string if exists else None (as string), demo convenience.
    cursor = _db.cursor()
    try:
        # Gets the password of the user if exists
        raw_data = cursor.execute(
...
def get_user_password(user: str) -> str | None:
    # WARNING: Demo purpose only. Real systems must never expose raw passwords.
    cursor = _db.cursor()
    try:
        raw_data = cursor.execute(
            "SELECT password FROM Users WHERE user=?;", (user,)
